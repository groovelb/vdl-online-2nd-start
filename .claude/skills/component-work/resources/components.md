...
- ProductCard: Lumenstate 제품 시그니처 카드. Day/Night 쌍 이미지 + timeValue smoothstep 블렌드 (brightness/saturate filter 연동, duration.slowest 1200ms), TIME_SLOTS 4시간대(점심/오후/저녁/밤) 연계, type Chip + 영문 타이틀 + 빛 메타(lux·kelvin), Framer Motion layoutId로 Shared Element 전이 준비 (`components/card/ProductCard.jsx`)
